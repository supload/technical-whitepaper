...
With a low threshold of allowing users to withdraw their Bitcoin earnings after only $1.00 USD, we need to think to the future about how to scale these payments. Users are currently incentivized to withdraw their earnings as soon as they reach the minimum withdraw threshold. With sufficient growth, Supload would be spamming the Bitcoin blockchain with many low $1.00 transactions.

Anything else is putting trust in a 3rd party(Supload) to hold their earnings until they withdraw. Which is bad cryptocurrency security practice.

The solution is to move these payments off of the primary Bitcoin blockchain and onto a secondary payment channel. We can update this payment channel as often as we like. The end user can then close the channel whenever they want, publishing the final transaction to the Bitcoin blockchain. This finalizes the transaction between Supload and the user, with no risk to the payee at any point.
...
The Lightning Network gives us the unprecedented ability to transact very small amounts of Bitcoin in near realtime,  for very little fees. This will be a feature that is available to the highest ranked users on Supload, as a few of our biggest content creators are responsible for a majority of the payments.

The Lightning Network will provide an extra layer of trust for users. As the payments are being updated, the payee is receiving a new signed Hashed Timelock Contract from Supload. With the committed amount of payment. At that point, it is impossible for the payer (Supload) to go back on the agreed upon payment amount. The funds are then being held in a contract on the Bitcoin blockchain. The amount owed to the content creator will update every 10 minutes when we run our analytics, adjusting for pageviews in that period of time.

When the user wants to finalize and receive their funds, they can close the payment channel with Supload. The user's wallet will then take the most current Timelock Contact and publish it to the Bitcoin blockchain. 

Therefore, only one transaction hits the Bitcoin blockchain and while the payment channel is open, the user is at no risk of not receiving their funds and wanting to withdraw after every $1 is accrued.
